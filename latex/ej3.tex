\section{Ejercicio 3}
    ...
    La idea es separar el problema principal en subproblemas. En este caso, el subproblema es, en base a la capacidad restante de cada mochila y al valor de cada tesoro, en qué mochila nos conviene meter cada uno (incluyendo la opción de no meterlo en ninguna). \par
    Por lo tanto, el algoritmo propuesto consta de una matriz por tesoro, de tamaño $(N+1)*(M+1)*(K+1)$ (suponiendo que hay tres mochilas), siendo éstas las capacidades de las mochilas M1, M2 y M3 respectivamente. Y además una matriz nula de igual dimensión que servirá para el caso en donde no hayan tesoros. \par
