\section{Ejercicio 3}
    ...
    \subsection{Descripción del problema}
		...
		Por cada tesoro, hay $C_{i}$ cantidades de cada tipo, cada uno tiene un valor $V_{i}$ y un peso $P_{i}$. Si bien el objetivo principal de la expedición era otro, nunca viene mal armarse de algún recuerdo. 
		Tienen M mochilas disponibles para llevarse los tesoros y cada una tiene un máximo de peso $K_{i}$ que puede llevar. \par
		El objetivo es, llevarse el mayor valor posible de tesoros. Para esto, se debe devolver como resultado el valor total de tesoros guardados y además, por cada mochila se debe aclarar, la cantidad de tesoros llevados y de qué tipo son.
        ...
        no es una salida válida, ya que se busca el máximo valor posible.
        
    % 2. Explicar de forma clara, sencilla, estructurada y concisa, las ideas desarrolladas para la resolución del problema. Utilizar pseudocódigo y lenguaje coloquial (no código fuente). Justificar por qué el procedimiento resuelve efectivamente el problema.
    \subsection{Solución propuesta}
    Para el problema planteado se uso la técnica algorítmica de \emph{Programacion dinámica}. El algoritmo propuesto consta de una matriz por tesoro, de tamaño $(N+1)*(M+1)*(K+1)$ (suponiendo que hay tres mochilas), siendo estas, las capacidades de las mochilas M1, M2 y M3 respectivamente, además de una matriz nula que servirá para el caso, en donde no hayan objetos. \par
    ...
    %FALTA QUE EL PSEUDOCODIGO SE VEA LINDO
